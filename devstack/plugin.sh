...
if is_service_enabled fuxi; then
    ...
    if [[ "$1" == "stack" && "$2" == "install" ]]; then
        ...
        # Run etcd first
        pgrep -x "etcd" && run_process etcd-server "$DEST/etcd/etcd-$ETCD_VERSION-linux-amd64/etcd --data-dir $DEST/etcd/db.etcd --advertise-client-urls http://0.0.0.0:$FUXI_ETCD_PORT  --listen-client-urls http://0.0.0.0:$FUXI_ETCD_PORT"
