...
if is_service_enabled fuxi; then
    ...
    if [[ "$1" == "stack" && "$2" == "install" ]]; then
        ...
        run_process docker-engine "/usr/bin/sudo /usr/bin/docker daemon -H unix://$FUXI_DOCKER_ENGINE_SOCKET_FILE -H tcp://0.0.0.0:$FUXI_DOCKER_ENGINE_PORT --cluster-store etcd://localhost:$FUXI_ETCD_PORT"
