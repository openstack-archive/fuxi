...
def get_mountpoint(volume_provider_type, volume_name, volume_id):
    try:
        vol_dir = os.path.join(CONF.volume_dir, volume_provider_type)
        if not os.path.exists(vol_dir) or not os.path.isdir(vol_dir):
            utils.execute('mkdir', '-p', '-m=700', vol_dir,
                          run_as_root=True)
        ...
        LOG.error(_LE("Unexpected error happened. Error: {0}").format(e))
        raise exceptions.FuxiException("Unexpected Error Happened")
