...
volume_providers_conf = {
    CINDER: 'fuxi.volumeprovider.cinderprovider.Cinder', }
...
def init_app_conf():
    ...
    try:
        ...
        if not os.path.exists(volume_dir) or not os.path.isdir(volume_dir):
            utils.psutil_execute('mkdir', '-p', '-m=700', volume_dir)
...
def volumedriver_path():
    ...
    for provide in app.volume_providers.values():
        try:
            mountpoint = provide.path(docker_volume_name)
            app.logger.info("Get mountpoint %(mp)s for docker volume "
                            "%(name)s" % {'mp': mountpoint,
...
def volumedriver_get():
    ...
    for provider in app.volume_providers.values():
        try:
            mountpoint = provider.show(docker_volume_name)
            volume = {'Name': docker_volume_name, 'Mountpoint': mountpoint}
