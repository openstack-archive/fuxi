...
def get_host_id():
    """Get an value that could represent this server.

    :return:
    """
    host_id = None

    volume_connector = CONF.volume_connector
...
class Cinder(provider.Provider):
    ...
    def _unmount(self, docker_volume_name, cinder_volume):
        ...
        if not link_path or not os.path.exists(link_path):
            msg_ft = _LE("Could not find device link path for volume {0} {1} "
        ...
        mountpoint = self._get_mountpoint(cinder_volume)
        ...
        try:
            ...
            for vol in self.cinderclient.volumes.list(search_opts=search_opts):
                ...
                for am in vol.attachments:
                    mountpoint = self._get_mountpoint(vol)
                    if ((_connector == OPENSTACK
                            and am['server_id'] == host_id)
    ...
    def mount(self, docker_volume_name):
        ...
        mountpoint = self._get_mountpoint(cinder_volume)
        blockdevice.create_mountpoint(mountpoint)
    ...
    def _get_mountpoint(self, cinder_volume):
        return os.path.join(CONF.volume_dir, self.volume_provider_type,
                            ''.join([cinder_volume.name,
                                     CONF.volume_joiner,
                                     cinder_volume.id]))
