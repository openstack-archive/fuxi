...
class Cinder(provider.Provider):
    ...
    def _create_volume(self, docker_volume_name, volume_opts):
        ...
        try:
            ...
            msg = _LE("Error happened when create an volume {} from Cinder. "
                      "Error: {}").format(docker_volume_name, e)
            LOG.error(msg)
            raise
        ...
        if CONF.volume_connector == OPENSTACK:
            try:
                ...
                msg = _LE("Error happened when remove docker volume "
                          "mountpoint directory. Error: {}").format(e)
                LOG.warn(msg)
    ...
    def list(self):
        ...
        try:
            ...
            LOG.error(_LE("Retrieve volume list failed. Error: {}").format(e))
            raise
    ...
    def _get_docker_volume(self, docker_volume_name):
        ...
        try:
            ...
            LOG.error(_LE("Error happened while getting volume list "
                          "information from cinder. Error: {}").format(ex))
            raise
