...
# Current supported and checked Manila share protocol
# NFS
...
class Manila(provider.Provider):
    ...
    def _create_share_network(self, volume_opts=None):
        neutron_net_id = volume_opts.get('neutron_net_id', None)
        neutron_subnet_id = volume_opts.get('neutron_subnet_id', None)

        if not neutron_net_id or not neutron_subnet_id:
            raise exceptions.InvalidInput(
                "Must provide share_network or both neutron_net_id "
                "and neutron_subnet_id")

        # If Share network already exists in Manila with provided neutron
        # net_info, then use it, otherwise create an new share network.
        share_nets = self.manilaclient.share_networks.list()
        for share_net in share_nets:
            if share_net.neutron_net_id == neutron_net_id \
                    and share_net.neutron_subnet_id == neutron_subnet_id:
                msg = _LW("Find matched share_network %(s_net)s with "
                          "neutron_net_id  %(n_net)s and "
                          "neutron_subnet_id %(n_subnet)s")
                LOG.warn(msg, {'s_net': share_net, 'n_net': neutron_net_id,
                               'n_subnet': neutron_subnet_id})
                return share_net

        try:
            share_network = self.manilaclient.share_networks.create(
                neutron_net_id=neutron_net_id,
                neutron_subnet_id=neutron_subnet_id)
        except manila_exception.ClientException as e:
            LOG.error(_LE('Create Manila network share failed. Error: %s'), e)
            raise

        return share_network

    def _create_share(self, docker_volume_name, share_opts):
        share_network_id = share_opts.get('share_network', None)
        if not share_network_id:
            share_network = self._create_share_network(share_opts)
            share_opts['share_network'] = share_network.id
    ...
    def create(self, docker_volume_name, volume_opts):
        try:
            ...
            if share:
                LOG.warn(_LW("Volume %(vol)s already exists in Manila, and "
    ...
    def _delete_share(self, share):
        try:
            ...
            if len(share_access_list) > 0:
                LOG.warn(_LE("Share %s is still used by other server, so "
    ...
    def mount(self, docker_volume_name):
        share, state = self._get_docker_volume(docker_volume_name)
        if state == NOT_ATTACH:
            LOG.warn(_LW("Find share %s, but not attach to this server, "
                         "so connect it"), share)
