...
def mount_device_ref_count(devpath):
    ref_count = 0
    mounts = FakeMounter().read_mounts()
    for m in mounts:
        if devpath == m.device:
...
class TestMounter(base.TestCase):
    ...
    def test_mount_device_ref_count(self):
        self.assertEqual(1, mount_device_ref_count('/dev/0'))

    def test_mountpoint_for_device(self):
        self.assertEqual('', get_mountpoint_for_device('/dev/0', '/path/to/1'))
        self.assertEqual('/path/to/1',
                         get_mountpoint_for_device('/dev/1', '/path/to/1'))
